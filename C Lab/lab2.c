...
void menu(char *** array, int * count, char * outFileName);
void countModule(int * count);
void searchModule(char *** array, int count);
int binarySearch(char * word, char *** array, int count);
void insertModule(char *** array, int * count);
void removeModule(char *** array, int * count);
int removeWord(char *** array, int location, int count);
void quitModule(char *** array, int count, char * outFileName);
int saveDictionary(char *** array, int count, char * outFileName);
...
int main(int argc, char * argv[]) {
    ...
    menu(&wordArray, &wordCount, argv[2]);
    freeDictionary(&wordArray, &wordCount);
    return 0;
}

/*
Perform the file input, reading, and closing. Allocate the main dictionary
and call the re-size if necessary. Get each line from the file and call
insertWord() appropriately. Print '.' as every 5000th word gets processed.
*/
int loadArray(char * inFileName, char *** array, int * count, int * capacity) {
    FILE *inFile;
    ...
    *count = 0;
    while (fscanf(inFile, "%s", word) == 1) {
    	if (*count >= *capacity) {
            /* call a function that will double the size of the array and
             copy its contents */
            repackage(array, capacity);
	    }
        // It's not erroneous to attempt to add duplicates, simply raise an
        // output message and halt the insertion
    	int insertOK = 0;
...
void menu(char *** array, int * count, char * outFileName) {
    ...
    for(;;) {
        ...
        switch(c) { // Spare pinkies (if human) and allow lower-case
            ...
            case 'P':
            case 'p':
                printArray(*array, *count); // No new module needed
                break;
            case 'S':
            case 's':
                // Call the search interface module
                searchModule(array, *count); 
                break;
            case 'I':
            case 'i':
                // Call the insert word module
                insertModule(array, count); // count will be modified, pass ptr
                break;
            case 'R':
            case 'r':
                // Call the remove word module
                removeModule(array, count); // count will be modified, pass ptr 
                break;
            case 'C':
            case 'c':
                // Return the current count
                countModule(count); break;
            case 'Q':
            case 'q':
                quitModule(array, *count, outFileName);
...
/*
Display the contents of the array that serves as the dictionary.
*/
void printArray(char ** array, int count) {
   printf("==========PRINT==========\n");
   int i=0;
...
void searchModule(char *** array, int count) {
    ...
    for (;;) {
        ...
        int exists = binarySearch(query, array, count);
        if (-1 != exists)
            printf("\n\t%s found.\n\n", query);
        else
            printf("\n\t%s not found.\n\n", query);
...
void insertModule(char *** array, int * count) {
    ...
    for (;;) {
        ...
        int insertStatus;
        insertStatus = insertWord(*array, count, query);
...
void removeModule(char *** array, int * count) { // Will need to modify count
    printf("==========REMOVE==========\n");
    for (;;) {
        printf("Enter a word(<Enter> to exit):\t");
        char query[256];
        ...
        int location = binarySearch(query, array, *count);
        if (-1 == location) {
            printf("\n\t%s ignored (not found)\n\n", query);
            continue;
        }
        if (removeWord(array, location, *count)) {
            (*count) -= 1;
            printf("\n\t%s removed\n\n", query);
        }
    }
...
void quitModule(char *** array, int count, char * outFileName) {
    ...
    if (saveDictionary(array, count, outFileName))
...
void freeDictionary(char *** array, int * count) {
    ...
    // Free the original dictionary itself
    free(*array);
}
